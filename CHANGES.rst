...
- Added :class:`~icalendar.cal.lazy.LazyCalendar` for lazy subcomponent parsing. This subclass of :class:`~icalendar.Calendar` defers parsing of VEVENT, VTODO, VJOURNAL, VFREEBUSY, and VAVAILABILITY components until they are accessed via ``.events``, ``.todos``, ``.walk()``, or other component methods. Calendar properties and VTIMEZONE components are still parsed eagerly. This is useful for accessing calendar metadata or counting events without full parsing overhead. See :issue:`1050`.
- Added :attr:`~icalendar.cal.calendar.Calendar.journals` property to :class:`~icalendar.Calendar` for API consistency with ``.events``, ``.todos``, and other component accessors. See :issue:`1050`.
