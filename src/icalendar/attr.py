...
def single_int_property(
    ...
    Parameters:
        ...
        min_value: The optional inclusive minimum allowed for the value.
    """

    def validate(value: int) -> int:
        if min_value is not None and value < min_value:
            raise ValueError(f"{prop} must be >= {min_value}")
        return value

    def fget(self: Component) -> int:
        """Get the property"""
        try:
            value = int(self.get(prop, default))
        except ValueError as e:
            raise InvalidCalendar(f"{prop} must be an int") from e
        try:
            return validate(value)
        except ValueError as e:
            raise InvalidCalendar(str(e)) from e
